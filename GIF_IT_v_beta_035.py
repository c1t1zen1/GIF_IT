...
def update_slider_label(label, value, scale):
    if scale == 10:
        value = "{:.1f}x".format(value)  # Add 'x' to the 'resample' value
    else:
        value = str(int(value))  # Scale the 'colors' value from 1 to 256
...
def create_custom_slider(text, from_, to, initial):
    ...
    label.pack(pady=1)
    frame = tk.Frame(window, bg='powder blue')  # Create a new frame for the slider and its value label
    frame.pack()
    canvas = tk.Canvas(frame, width=175, height=50, bg='powder blue', highlightthickness=0)  # Set highlightthickness to 0
    canvas.pack(side='left')  # Pack the canvas to the left of the frame
    ...
    value_label = tk.Label(frame, text=str(initial), bg='powder blue', font=('TkDefaultFont', 9, 'bold'))
    value_label.pack(side='left', padx=1)  # Pack the value label to the left of the frame with a padding of 5 pixels
    def update_circle(event):
        x = event.x
        if 50 <= x <= 150:
            canvas.coords(circle, x-5, 20, x+5, 30)
            update_slider_label(value_label, from_ + (to - from_) * (x - 50) / 100, scale)
