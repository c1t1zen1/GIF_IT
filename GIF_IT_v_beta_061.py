...
# ---------------------------------------------------------------------------
# Try to import imageio for MP4 / H.265 export
# ---------------------------------------------------------------------------
try:
...
FFMPEG_DIR = CONFIG_DIR / "ffmpeg"
FFMPEG_EXE = FFMPEG_DIR / ("ffmpeg.exe" if sys.platform == "win32" else "ffmpeg")

# URL for gyan.dev release essentials build (Windows, includes libx265)
FFMPEG_DOWNLOAD_URL = (
...
def find_ffmpeg() -> str | None:
    ...
    # 1. Check cached download in ~/.gif_it/ffmpeg/
    if FFMPEG_EXE.is_file():
        return str(FFMPEG_EXE)
    # 2. imageio-ffmpeg (works when running from source with pip install)
...
def download_ffmpeg(progress_callback=None) -> str:
    """Download ffmpeg essentials build and cache to ~/.gif_it/ffmpeg/.

    Returns the path to ffmpeg.exe on success.
    Raises RuntimeError on failure.
    ``progress_callback(percent: float)`` is called with 0-100 values.
    """
    FFMPEG_DIR.mkdir(parents=True, exist_ok=True)
    tmp_zip = FFMPEG_DIR / "ffmpeg_download.zip"
    ...
    try:
        ...
        if FFMPEG_EXE.is_file():
            FFMPEG_EXE.unlink()
        raise RuntimeError(f"Failed to download ffmpeg: {e}") from e
...
class GifItApp(ctk.CTk):
    ...
    def _prompt_ffmpeg_download(self):
        """Ask the user to download ffmpeg, then retry creation."""
        ok = messagebox.askyesno(
            "ffmpeg Required",
            "MP4 export requires ffmpeg, which is not installed.\n\n"
            "Download ffmpeg now? (~100 MB, one-time download)\n"
            "It will be cached for future use.",
        )
        if not ok:
            return
        # Show download progress dialog
        self._dl_dialog = ctk.CTkToplevel(self)
        self._dl_dialog.title("Downloading ffmpeg...")
        self._dl_dialog.geometry("350x100")
        self._dl_dialog.resizable(False, False)
        self._dl_dialog.transient(self)
        self._dl_dialog.grab_set()
        ctk.CTkLabel(self._dl_dialog, text="Downloading ffmpeg...").pack(pady=(12, 4))
        self._dl_progress = ctk.CTkProgressBar(self._dl_dialog, width=300)
        self._dl_progress.pack(pady=4, padx=20)
        self._dl_progress.set(0)
        self._dl_pct_label = ctk.CTkLabel(self._dl_dialog, text="0%")
        self._dl_pct_label.pack()

        def _on_progress(pct):
            self.after(0, lambda p=pct: self._dl_update(p))

        def _worker():
            try:
                download_ffmpeg(progress_callback=_on_progress)
                self.after(0, self._dl_success)
            except Exception as e:
                self.after(0, self._dl_fail, str(e))

        threading.Thread(target=_worker, daemon=True).start()

    def _dl_update(self, pct: float):
        self._dl_progress.set(pct / 100)
        self._dl_pct_label.configure(text=f"{int(pct)}%")

    def _dl_success(self):
        self._dl_dialog.destroy()
        messagebox.showinfo("Download Complete", "ffmpeg installed successfully!")
        # Retry creation now that ffmpeg is available
        self._on_create()

    def _dl_fail(self, error: str):
        self._dl_dialog.destroy()
    ...
    def _register_tour_steps(self):
        ...
        self.tour.add_step(self.format_menu,
                           "FORMAT — Export as GIF or MP4 (H.265).")
