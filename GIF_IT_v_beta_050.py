...
def create_gif_from_images(images, output_folder, output_name, folder_path, speed=100, dissolve=0, resample=1, colors=256):
...
def create_gif(folder_path=None):
    ...
    if folder_path:
        ...
        # Save images as GIF
        output_path = os.path.join(output_folder, output_name + '.gif')
        images[0].save(output_path, save_all=True, append_images=images[1:], duration=gif_speed, loop=0)
