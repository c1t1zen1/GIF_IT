...
# Create a style
style = ttk.Style()
style.configure("blue.Horizontal.TScale", background='powder blue', troughcolor='powder blue')

# Function to update the label of a scale widget
def update_scale_label(label, value):
    label.config(text="{:.1f}".format(float(value)))

# Add a label and slider for resample
resample_label = tk.Label(window, text="RESAMPLE IT", bg='powder blue', font=('TkDefaultFont', 9, 'bold'))
resample_label.pack(pady=2)
resample_value_label = tk.Label(window, text="1.0", bg='powder blue', font=('TkDefaultFont', 9, 'bold'))
resample_value_label.pack()
resample_slider = ttk.Scale(window, from_=0.1, to=10, length=125, style="blue.Horizontal.TScale", command=lambda value: update_scale_label(resample_value_label, value))  # Use the ttk.Scale widget with the custom style
resample_slider.set(1)
resample_slider.pack()

# Add a label and slider for colors
colors_label = tk.Label(window, text="COLORS IT", bg='powder blue', font=('TkDefaultFont', 9, 'bold'))
colors_label.pack(pady=2)
colors_value_label = tk.Label(window, text="256", bg='powder blue', font=('TkDefaultFont', 9, 'bold'))
colors_value_label.pack()
colors_slider = ttk.Scale(window, from_=1, to=256, length=125, style="blue.Horizontal.TScale", command=lambda value: update_scale_label(colors_value_label, value))  # Use the ttk.Scale widget with the custom style
